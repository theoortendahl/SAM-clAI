...
data_dir="training/dataset/" # Path to dataset
data=[] # list of files in dataset
for ff, name in enumerate(os.listdir(data_dir+"imgs/")):  # go over all folder annotation
    data.append({"image":data_dir+"imgs/"+name,"annotation":data_dir+"masks/"+'mask'+os.path.splitext(name)[0][3:]+'.png'})
...
model_cfg = "./configs/sam2/sam2_hiera_s.yaml" #  model config
...
for itr in range(80001):
    ...
    with torch.amp.autocast('cuda'): # cast to mix precision
            ...
            print("step)",itr, "Accuracy(IOU)=",mean_iou, flush=True)


            if itr % 1000 == 0: 
                checkpoint = {
                'iteration': itr,  # Save the current iteration
                'model_state_dict': predictor.model.state_dict(),  # Model weights
                'optimizer_state_dict': optimizer.state_dict(),  # Optimizer state
                'scaler_state_dict': scaler.state_dict(),  # Mixed precision scaler state
                'mean_iou': mean_iou  # Save the current mean IOU
                }
                torch.save(checkpoint, f"checkpoints/sam2_checkpoint_{itr}.pt")
